...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	...
	if(!empty($multipage)){
		...
		$curr_url = pageurl($urlrule, $curr_page, $array);
		if(!empty($array['categorydir'])){//生成html的处理方式
			//var_dump($multipage);exit;
			$curr_url = dirname($curr_url).'/';
			$multipage.='<script>function isInteger( str ){ var regu = /^[0-9]{1,}$/; return regu.test(str);}
				</script>&nbsp;<input type="text" value='.$curr_page.' name="custompage" id="custompage"  onkeydown="if(event.keyCode==13) {if(isInteger(this.value) && this.value<=1) window.location=\''.$curr_url.'\'+(this.value)+\'.html\'; return false;}" />&nbsp;&nbsp;<input type="button" class="go_btn" value="go" onclick="if(isInteger(document.getElementById(\'custompage\').value) && document.getElementById(\'custompage\').value>1) window.location=\''.$curr_url.'\'+(document.getElementById(\'custompage\').value)+\'.html\'" />';
		}else{
			$pos = strpos($curr_url,'&page');
			if($pos!==false){
				$curr_url=  substr($curr_url,0,$pos);
			}
			//var_dump($multipage);exit;
			$multipage.='<script>function isInteger( str ){ var regu = /^[0-9]{1,}$/; return regu.test(str);}
				</script>&nbsp;<input type="text" value='.$curr_page.' name="custompage" id="custompage"  onkeydown="if(event.keyCode==13) {if(isInteger(this.value)) window.location=\''.$curr_url.'&page='.'\'+(this.value); return false;}" />&nbsp;&nbsp;<input type="button" class="go_btn" value="go" onclick="if(isInteger(document.getElementById(\'custompage\').value)) window.location=\''.$curr_url.'&page='.'\'+(document.getElementById(\'custompage\').value)" />';
		}
	}//end 2015/12/26
	return $multipage;
}
...
function pageurl($urlrule, $page, $array = array()) {
	...
	$findme = array('{$page}','{$endir}');//自定义参数，分页需要在此做替换 2016/1/11
	$replaceme = array($page,'en');

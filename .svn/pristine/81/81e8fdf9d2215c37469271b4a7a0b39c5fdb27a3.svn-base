...
class index extends foreground {
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			if(empty($_SESSION['connectid']) && !$_POST['quick_login']) {
				//判断验证码
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if(!$r) showmessage(L('user_not_exist'),HTTP_REFERER);//'index.php?m=member&c=index&a=login'
				...
				if($r['password'] != $password) {				
					...
					showmessage(L('password_error', array('times'=>$times)),HTTP_REFERER, 3000);
				}
			...
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) :HTTP_REFERER ;
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			//'index.php?m=member&c=index&a=login'
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : HTTP_REFERER;

 ...
 * @param $modelid 模型ID
 */
function filters($field,$modelid,$diyarr = array(),$diy_url_param='') {
 $fields = getcache('model_field_'.$modelid,'model');
 ...
 $option[$k]['menu'] = $field_value == $k ? '<em>'.$v[0].'</em>' : '<a href='.$option[$k]['url'].$diy_url_param.'>'.$v[0].'</a>' ;
 }
 $all['name'] = '全部';
 $all['url'] = structure_filters_url($field,array($field=>''),2,$modelid);
 $all['menu'] = $field_value == '' ? '<em>'.$all['name'].'</em>' : '<a href='.$all['url'].$diy_url_param.'>'.$all['name'].'</a>';

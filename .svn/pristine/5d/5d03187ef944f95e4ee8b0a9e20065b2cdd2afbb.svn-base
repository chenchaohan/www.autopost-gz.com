...
class url{
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			$urls = str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}','{$endir}'),array($categorydir,$catdir,$year,$month,$day,$catid,$id,$page,$endir),$urlrule);
	...
	public function category_url($catid, $page = 1) {
		...
		if (!$setting['ishtml']) { //如果不生成静态
			...
			$endir = 'en';
			$urls = str_replace(array('{$categorydir}','{$catdir}','{$catid}','{$page}','{$endir}'),array($category_dir,$category['catdir'],$catid,$page,$endir),$urlrule);
			if ($url && $domain_dir) { //如果存在设置二级域名的情况
				if (strpos($urls, $domain_dir)===0) {
					//2016/1/6
					if (in_array('{$endir}', explode('/',$urlrule))) {
						$url = $url.$urls;
						//var_dump(1111,$urls,$url);
					}else{
	...
	public function get_list_url($ruleid,$categorydir, $catdir, $catid, $page = 1) {
		...
		$endir = 'en';
		$urls = str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$page}','{$endir}'),array($categorydir,$catdir,$year,$month,$day,$catid,$page,$endir),$urlrule);
		return $urls;
	}
